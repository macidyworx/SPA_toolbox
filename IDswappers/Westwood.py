"""
Westwood.py - ID Swapper for Westwood Maths Excel files.

Files have a tab called 'Westwood Maths' containing the student IDs to swap.
Column headers: Surname, First_Name, Student ID.
...
# Tab to process
TARGET_SHEET = "Westwood Maths"


class WestwoodSwapper:
    """Processes Westwood Maths Excel files and swaps student IDs based on SIF or SSOT."""

    def __init__(self):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(script_dir, "logging.ini")
        self.logger = THElogger(script_name="Westwood", config_file=config_path)
    ...
    def _process_xlsx_sheet(self, ws, mode, lookup, not_found, file_path):
        """
        Process the Westwood Maths openpyxl worksheet.
    ...
    def _process_xlrd_sheet(self, rb_sheet, wb_sheet, mode, lookup, not_found, file_path):
        """
        Process the Westwood Maths xlrd sheet.
    ...
    def run(self):
        ...
        output_dir = select_output_folder("Select output folder for Westwood")
        if not output_dir:
            self.logger.info("User cancelled output folder selection.")
            self.logger.finalize_report()
            return

        ww_folder = os.path.join(output_dir, "Westwoodswapped")

        # Handle existing output subfolder
        if os.path.exists(ww_folder):
            result = wx.MessageBox(
                f"{ww_folder} already exists.\nRemove it and continue?",
                "Output Folder Exists", wx.YES_NO | wx.ICON_WARNING)
            if result == wx.YES:
                shutil.rmtree(ww_folder)
            else:
        ...
        skipped_folder = os.path.join(ww_folder, "SKIPPED")
        ...
        for file in files:
            ...
            if file.lower().endswith('.xlsx'):
                ...
                # Save to Westwoodswapped
                output_path = os.path.join(ww_folder, os.path.basename(file))
                try:
                    wb.save(output_path)
        ...
        if not_found:
            ...
            log_wb.save(os.path.join(ww_folder, "not_found_log.xlsx"))
            log_wb.close()
        ...
        if not_found or files_checked or files_skipped:
            ...
            report_wb.save(os.path.join(ww_folder, "Westwood_report.xlsx"))
            report_wb.close()
        ...
        self.logger.info(f"Processing complete. Files saved in {ww_folder} folder.")
        self.logger.finalize_report()
