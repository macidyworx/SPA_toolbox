...
class RRSwapper:
    """Processes Running Records Excel files and swaps student IDs based on SIF or SSOT."""

    def __init__(self):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(script_dir, "logging.ini")
        self.logger = THElogger(script_name="RR", config_file=config_path)
    ...
    def _process_xlsx_sheet(self, ws, sheet_name, mode, lookup, not_found, file_path):
        ...
        if headers is None:
            self.logger.warning(f"Required headers not found in tab '{sheet_name}' of {os.path.basename(file_path)}")
            return (0, 0, 0)
    ...
    def run(self):
        ...
        output_dir = select_output_folder("Select output folder for RR")
        if not output_dir:
            self.logger.info("User cancelled output folder selection.")
            self.logger.finalize_report()
            return

        rr_folder = os.path.join(output_dir, "RRswapped")

        # Handle existing output subfolder
        if os.path.exists(rr_folder):
            result = wx.MessageBox(
                f"{rr_folder} already exists.\nRemove it and continue?",
        ...
        skipped_folder = os.path.join(rr_folder, "SKIPPED")
        ...
        for file in files:
            ...
            if file.lower().endswith('.xlsx'):
                ...
                available_sheets = [s for s in TARGET_SHEETS if s in wb.sheetnames]
                if not available_sheets:
                    self.logger.warning(f"No target tabs found in {os.path.basename(file)}. Skipping.")
                ...
                # Save to RRswapped
                output_path = os.path.join(rr_folder, os.path.basename(file))
                try:
                    wb.save(output_path)
        ...
        if not_found:
            ...
            log_wb.save(os.path.join(rr_folder, "not_found_log.xlsx"))
            log_wb.close()
        ...
        if not_found or files_checked or files_skipped:
            ...
            report_wb.save(os.path.join(rr_folder, "RR_report.xlsx"))
            report_wb.close()
        ...
        self.logger.info(f"Processing complete. Files saved in {rr_folder} folder.")
        self.logger.finalize_report()
