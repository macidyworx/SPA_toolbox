...
TESTING = True
TESTING_FILES_FOLDER = os.path.join(repo_root, 'ZZSample_Files', 'Test_Files', 'EOI Insight')
...
Description:
This script processes EOI Template Excel files (.xls and .xlsx) in a specified folder and its
subfolders, swaps student IDs based on a Student Information File (SIF), and
saves modified copies to an output folder named 'EOIswapped'.
...
3. Prompts for the output directory where 'EOIswapped' will be created.
4. Loads the SIF file into a DataFrame.
5. Recursively finds all .xls/.xlsx files in the input folder.
6. For each file:
   - Finds the 'First Name', 'Last Name', and 'ID' columns in the active sheet.
...
7. Saves modified files to the EOIswapped folder.
8. Saves a log of not-found students as 'not_found_log.xlsx' in EOIswapped.

Requirements:
- Python with pandas, openpyxl installed.
- SIF file must have columns: 'Surname', 'Firstname', 'StudentID'.
- Input files must have 'First Name', 'Last Name', and 'ID' headers in the first sheet.

Usage:
Run the script: python EOI.py
...
def get_user_inputs():
    ...
    output_dir = input(f"Enter the output directory where EOIswapped will be created (e.g., {example_output}): ").strip('"').strip("'")
...
eoi_folder = os.path.join(output_dir, "EOIswapped")
while os.path.exists(eoi_folder):
    print("")
    print("===================================================================================================")
    print(f"Error >> {eoi_folder} already exists.")
    print("")
    choice = input(f"Do you want to (r)emove it, (m)ove to a new location, or (q)uit? (r/m/q): ").lower().strip()
    if choice == 'r':
        shutil.rmtree(eoi_folder)
        print(f"Removed {eoi_folder}.")
    elif choice == 'm':
        new_dir = input("Enter new output directory: ")
        eoi_folder = os.path.join(new_dir, "EOIswapped")
        print(f"Changed to {eoi_folder}.")
    elif choice == 'q':
        print("Exiting.")
        exit()
    else:
...
parent_dir = os.path.dirname(eoi_folder)
...
skipped_folder = os.path.join(eoi_folder, "SKIPPED")
...
for file in files:
    ...
    if '.xlsx' in file.lower():
        ...
        # Save to EOIswapped
        output_path = os.path.join(eoi_folder, os.path.basename(file))
        try:
            wb.save(output_path)
...
if not_found or files_checked or files_skipped:
    ...
    with pd.ExcelWriter(os.path.join(eoi_folder, "EOI_report.xlsx")) as writer:
...
print(f"Processing complete. Files saved in {eoi_folder} folder.")