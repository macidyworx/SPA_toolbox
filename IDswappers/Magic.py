"""
Magic.py - ID Swapper for Magic Words Excel macro-enabled files (.xlsm).

Files have a tab called 'MagicWords' containing the student IDs to swap.
Column headers: Surname, First_Name, Student_ID.
...
FILE_LNAME = "Surname"
FILE_ID_HEADER = "Student_ID"
SIF_SURNAME = "Surname"
SIF_FIRSTNAME = "Firstname"
SIF_STUDENTID = "StudentID"

# Tab to process
TARGET_SHEET = "MagicWords"


class MagicSwapper:
    """Processes Magic Words .xlsm files and swaps student IDs based on SIF or SSOT."""

    def __init__(self):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(script_dir, "logging.ini")
        self.logger = THElogger(script_name="Magic", config_file=config_path)
    ...
    def _process_sheet(self, ws, mode, lookup, not_found, file_path):
        """
        Process the MagicWords openpyxl worksheet.
    ...
    def run(self):
        ...
        files = select_work_files([".xlsm"])
        if not files:
            self.logger.info("No working files selected.")
            self.logger.finalize_report()
            return

        output_dir = select_output_folder("Select output folder for Magic Words")
        if not output_dir:
            self.logger.info("User cancelled output folder selection.")
            self.logger.finalize_report()
            return

        magic_folder = os.path.join(output_dir, "Magicswapped")

        # Handle existing output subfolder
        if os.path.exists(magic_folder):
            result = wx.MessageBox(
                f"{magic_folder} already exists.\nRemove it and continue?",
        ...
        os.makedirs(magic_folder, exist_ok=True)
        skipped_folder = os.path.join(magic_folder, "SKIPPED")
        ...
        for file in files:
            ...
            # openpyxl handles .xlsm with keep_vba=True to preserve macros
            ...
            ws = wb[TARGET_SHEET]
            checked, matched, nf = self._process_sheet(
            ...
            # Save to Magicswapped (preserve .xlsm extension and macros)
            output_path = os.path.join(magic_folder, os.path.basename(file))
            try:
                wb.save(output_path)
        ...
        if not_found:
            ...
            log_wb.save(os.path.join(magic_folder, "not_found_log.xlsx"))
            log_wb.close()
        ...
        if not_found or files_checked or files_skipped:
            ...
            report_wb.save(os.path.join(magic_folder, "Magic_report.xlsx"))
            report_wb.close()
        ...
        self.logger.info(f"Processing complete. Files saved in {magic_folder} folder.")
        self.logger.finalize_report()
...
___  ___            _        _    _               _
|  \/  |           (_)      | |  | |             | |
| .  . | __ _  __ _ _  ___  | |  | | ___  _ __ __| |___
| |\/| |/ _` |/ _` | |/ __| | |/\| |/ _ \| '__/ _` / __|
| |  | | (_| | (_| | | (__  \  /\  / (_) | | | (_| \__ \
\_|  |_/\__,_|\__, |_|\___|  \/  \/ \___/|_|  \__,_|___/
               __/ |            ___________
              |___/             |_   _|  _  \
